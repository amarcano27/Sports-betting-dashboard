...
st.title("🏒 NHL Model")
st.caption("Puck Line Structural Analysis")

games = load_nhl_games()
if not games:
    st.warning("No NHL games found in database.")
    st.stop()
...
for idx, g in enumerate(games):
    ...
    rec = "TRAP" if (away_ml < -150 or home_ml < -150) else ("VALUE" if best_e >= 1.5 else "LEAN")
